...
class Argument {
	...
	async obtain(msg, val, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			const pattern = msg.client.dispatcher.buildCommandPattern((msg.guild)? msg.guild.commandPrefix: '');

			if(val.match(pattern) && ( msg.client.registry.commands.filter(c => c.name === val.match(pattern)[2]).size || msg.client.registry.commands.filter(c => c.aliases && c.aliases.length).filter(c => c.aliases.includes(val.match(pattern)[2])).size)) {
				return {
					value: null,
					cancelled: 'cantRead',
					prompts,
