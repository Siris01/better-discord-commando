...
class CommandDispatcher {
	...
	shouldHandleMessage(message, oldMessage) {
		...
		const pattern = message.client.dispatcher.buildCommandPattern((message.guild)? message.guild.commandPrefix: '');
		const val = message.content;

		if (val.match(pattern) && ( message.client.registry.commands.filter(c => c.name === val.match(pattern)[2]).size || message.client.registry.commands.filter(c => c.aliases && c.aliases.length).filter(c => c.aliases.includes(val.match(pattern)[2])).size)) return true;
